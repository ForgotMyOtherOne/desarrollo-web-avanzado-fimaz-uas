...
header("Access-Control-Allow-Methods: Content-Type, Authorization, X-Requested-With");
...
if ($method === "POST" && count($segments) === 1) {
    ...
    if (!isset($data->existencia) || $data->existencia < 0) {
        $errores[] = "La existencia no puede ser negativo";
    }

    // VALIDACION PRO ADICIONAL: El precio de venta no puedo ser menor al de compra
...
if ($method === "PUT" && count($segments) === 2) {
    ...
    if ($data->precioVenta < 0) {
        $errores[] = "Precio de venta inválida";
    }
